...
- **0x60 (status)**: 常にステータス。bit0 = DRDY (読み戻し FIFO 非空)、
  ...
  ポールに落ちるのを防ぐ、従来どおり)
- **0x62 (data)**: CSRR/LPEN の結果バイトを 1 読み 1 バイトでポップ

CSRR (0xE0) は CSRW の 3 バイト (上位 &3) + ゼロ 2 バイトの 5 バイト、
LPEN (0xC0) は 3 バイトゼロ (ペンなしの実機挙動)。深さ 8、空き不足時は
再発行分を落とす (実チップの挙動)。tb_pc98_gdc に一連の試験追加、PASS。
