...
3. **ステータス遷移(1→2→3→4)はOSが観測する**。定数1だと
   ...
   ステータス以外の要因も混在)。
4. **rbf_r はバイトごとのビット順反転(リバース)必須**(反転なし→Load error General error)。
   検証方法: 先頭非FFバイトが `56 56 56 56 6c 2f` になっていること。
   **注意: 「反転」は XOR 0xFF ではない。各バイトのビット順を前後反転する**
   (`0x6a→0x56`, `0x36→0x6c`, `0xf4→0x2f`)。CI の生 rbf は
   `FF×128 + 6a 6a 6a 6a 36 f4` で始まり、ビット順反転すると正規形式
   `FF×128 + 56 56 56 56 6c 2f` になる(SD内278コアすべてがこの形式)。
   実証(2026-09-07): `bitrev(build/b1/ap_core.rbf) == dist/testB1/.../bitstream.rbf_r`
   が **全 1,741,492 バイト完全一致**(実機 BIOS 動作実績ペアで検証済み)。
   XOR 0xFF に読み替えると `00×128` 始まりになり実機でロード不可。
   変換コードは `scripts/package.sh`(translate テーブル)。
...
2. ~~**P0 手順5: 実機統合の下準備**~~ ✅ **完了**
   ...
   - **testB2(run#46、両バグ修正済み sdram_mp 版)を 2026-09-07 12:13 に SD へ投入済み。
      実機 A/B 再テスト待ち**(判定基準は下記「testB2 実機テスト」)
   - シムは 42.95MHz 据え置き。85.9MHz 化はクロック配線が CHIPSET〜core_top まで
     波及して検証の変数が増えるため分離した(Fmax は Fitter で別途評価)

     **testB2 実機テスト(SD投入済み・実施待ち)**
     - ビットストリーム: **run#46**(両バグ修正済み `sdram_mp`、`SDRAM_USE_MP=1`)。
       12,127 ALM / 66%、M10K 193 同一、Error 0。simジョブ全緑
       (`tb_ram_ab` 参照・シム両モード含む)
     - パッケージ: testB1 の JSON/boot.bin 流用、bitstream.rbf_r のみ差し替え
       (ビット順反転変換は testB1 ペアで100%一致検証済み)
     - **結果(2026-09-07 12:30頃): ❌ BIOS 出ず。** 2バグ修正では足りなかった
     - 判定基準に従い KFSDRAM 回帰ビルド(run#49)→ **testB3 を投入済み**(下記)

     **ボードタイミング実験(2026-09-07、testB2失敗の切り分け)**
     - 実機とシミュレーションの構造的差の筆頭は **dram_clk が逆相(180°)**:
       `pll.v` outclk_2 = 42.954545 MHz + phase_shift2 11640ps = 半周期遅延
       (VCO 859.09MHz の10タップ=出力周期の半分。コメントの @180deg は正しい)
     - `sim/sdram_board_model.sv` を新設: モデルのクロックを半周期遅延にし、
       コマンド/データに片道 7-8ns のフライト遅延を追加
     - **結果: KFSDRAM 参照も mp も PASS。** 逆相クロック・遅延込みでも
       mp+shim のリード経路は実機条件で正しい(RD_DELAY=CL+1 の解析とも一致)
     - 過程でモデルの不備を1件修正: 実部品はバーストリードのデータを
       **次のデータで置換されるまで保持**するが、モデルは1スロットでZに戻していた。
       逆相条件下ではこれが両コントローラを誤ってFAILさせる(修正で両方PASS、
       既存TB全て引き続きPASS)
     - **結論: リードタイミング仮説は否定。原因は別にある**
     - CI に `tb_ram_ab_ph`(ボードタイミング版)を両モードで回帰登録

     **testB3 実機テスト — ✅ POST 確認(2026-09-07)**
     - run#49 = **現行ツリー + 純KFSDRAM** が BIOS POST。
       **ツリーとKFSDRAM経路は健全。失敗は sdram_mp/shim 固有と確定**
       (全シミュレーション — コントローラ直/RAM.sv統合/ボードタイミング逆相込み —
       の外側にあるバグ)
     - STA比較: testB2 -0.903 / testB3 -0.266(同一ドメイン general[3]。
       B1は -0.728 で動作実績 → 決定的な差ではない)

     **testB4 = 実機バイセクト第1段(SD投入待ち)**
     - run#51 = **shimのグルー + 純正KFSDRAMをmpインタフェースに変換した末端**
       (`SDRAM_MP_KF_REF`。mpは完全排除、12,233 ALM)
     - 3モード(ref / mp / mp_kfref)とも tb_ram_ab PASS、CI も全緑
     - トランスレータは enable_refresh を KFSDRAM へ直結(0固定にすると
       モデルのリフレッシュ間隔チェックに引っかかる = 純正挙動からの逸脱を
       ローカルシムが検出。ハードテスト1回分を節約)
     - **判定**:
       - ✅ BIOS が出る → **犯人は sdram_mp 内部**。次は mp の内部を
         KFSDRAM 挙動に1ステップずつ近づけるバイセクト(INIT手順→リフレッシュ
         設計→RW/パイプライン)
       - ❌ BIOS が出ない → **犯人は shim のグルー(RAM.sv向けハンドシェイク)**。
         グルーの busy/flag/idle 生成を実機で切り分け
