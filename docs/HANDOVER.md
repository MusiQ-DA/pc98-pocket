...
### 1.0 結論: **CPU の SDRAM 読み出しが化けている**。これが根本原因。
...
4バイトでは「良いワードの隣に悪いワードが1個」までしか分からない。
交互なのか、連続なのか、単発なのかで原因が変わる。
`post_monitor` の窓を **16バイト(`F000:D880-D88F`)** に広げた(run#102)。

イメージの実体: `F8 2E E8 D2 EF 50 E3 F2 E6 6E FE 53 FF 53 FF A4`

| 見え方 | 示唆 |
|---|---|
| 1ワードだけ壊れている | 単発 — リフレッシュ衝突など、周期的なイベントとの決定論的な衝突 |
| ワード単位で交互 | 状態機械のパイプライン段ずれ |
| 途中から全部壊れる | ロウ/バンク境界(ただし §1.1 でこの4バイトには境界が無い) |
