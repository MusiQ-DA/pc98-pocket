...
残る作業は**キーボード入力をベンチに流し込むこと** — プロンプトに
`3`+CR を答えれば BASIC は先へ進む。
...
1. **キーボード入力をベンチに流す** — `How many files (0-15)?` に答える。
   ベンチには既にキーボードモデル(ACK 80ms 遅延)があるので、スキャンコードを
   注入する plusarg(`+keys=3\r` のような形)を足すのが最短。
   その後 BASIC は INT 1Ch AH=02 で 100Hz タイマを起動する(§3.7)ので、
   §3.2 の3修正がそこで初めて効く — **ここが修正の本当の検証点**。
2. `+golden` と PIT シードのフックを張り直すか撤去する(§3.5)。
   素のブートで BASIC が出たので、どちらも不要な可能性が高い。
3. 実機デプロイ: `bash scripts/deploy_pc98.sh`(ROM は md5 ピン留め済み)。
   実機ではキーボードが本物なのでプロンプトに人間が答えられる。
