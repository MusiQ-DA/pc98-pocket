...
### 1.2 PC-98 の実装状況(2026-09-09 04:00 時点)

| 部品 | 状態 |
|---|---|
| メモリマップ / BIOS 96KB → `E8000` | ✅ 接続済み |
| ITF バンク(ポート `0x043D`、`F8000-FFFFF`) | ✅ 接続済み |
| `pc98_tvram`(`A0000` 文字 / `A2000` 属性) | ✅ 接続済み |
| `pc98_font_ank`(8×16 ANK、BRAM) | ✅ 接続済み |
| `pc98_text_render`(属性、色は **GRB 順**) | ✅ 接続済み |
| `pc98_video_timing`(848×440 / 640×400 / 24,826Hz / 56.42Hz) | ✅ 接続済み |
| `pll_video_pc98`(21.0526 MHz) | ✅ 接続済み |
| `pc98_glyph_addr` | ✅ 実 `font.rom` で検証 |
| `pc98_glyph_rowbuf`(80セル×16バイト、ダブルバッファ) | ✅ 単体検証 |
| `pc98_font_fetch`(SDRAM バースト) | ✅ 通し検証(rowbuf→SDRAM) |
| shim の **port B**(2ポート化) | ✅ 全 SDRAM ベンチで退行なし |
| FONT.ROM を SDRAM へロード(フォントバンク) | ✅ 接続済み |
| **rowbuf → レンダラ接続(CDC 込み)** | ✅ **完了。run#122 で合成成功** |
| `pulse_cdc`(トグル同期) | ✅ 6種のクロック比で検証 |
| `pc98_cgwindow`(`A4000-A4FFF`) | ✅ 単体検証。**未接続** |

#### 1.2b 完成したテキストパス(run#122、CI 全緑)

```
ゲスト → TVRAM(A0000 文字 / A2000 属性)
              ├─ fil ポート(chipset 42.95MHz)→ pc98_glyph_rowbuf
              │                                    ↓ pc98_glyph_addr
              │                                  pc98_font_fetch
              │                                    ↓ shim port B(16ワードバースト)
              │                                  SDRAM の FONT.ROM(0x400000〜)
              │                                    ↓ 80セル×16バイト、ダブルバッファ
              └─ vid ポート(dot 21.05MHz)→ pc98_text_render → VGA_R/G/B
                                              ↑ グリフはここから
```

- **行単位フェッチ**: 1セル16バイトは連番なので1バーストで取れ、16走査線ぶん使い回す。
  走査線ごとにやるとランダム80アクセス = ライン時間の32%になる
- **CDC**: テキスト行の先頭はドットドメインで発生し、充填はチップセットドメイン。
  `pulse_cdc`(トグル同期)で渡す。**素の3段同期はダメ** — 遅いクロックへは
  パルスを落とし、速いクロックへは2回に増える。都合のいいクロック比の
  シミュレーションでは見えず、実機で間欠故障になる
- **行バッファは「レンダラが取りに行っているセル」で引く**。レンダラは1セル先読み
  するので、現在の列で引くと行全体が1セルずれる(`font_cell` を出させている)

##### この段で Quartus が捕まえた本物のバグ

`tvram_vid_cell_w` を**レンダラと行バッファの両方が駆動**していた
(1ポートに2マスタ、しかも別クロックドメイン)。
両者は欲しいものが違う(レンダラ=属性、行バッファ=文字コード)ので、
**ポートを分けた**。文字バンクと属性バンクは元々別配列なので**重複コストはゼロ**。

→ レンダラは文字コードを一切見なくなり、漢字判定も不要になった
(`kanji_seen` はペアリングを知っている行バッファへ移動)。

#### 1.2e 残っている接続作業(CG ウィンドウ)

`pc98_cgwindow` は作ってテスト済みだが**まだ繋いでいない**。必要なのは:

1. **ポート 0xA1/0xA3/0xA5 の書き込みを拾う** — `Peripherals.sv` の
   I/O デコードに追加。AEN 修飾と2サイクル安定化を忘れないこと
   (ITF バンクと同じ理由。ROM を切り替えるほどの被害は無いが、
   窓が化けるとゲストが誤った字形を読む)
2. **`A4000-A4FFF` の読み出しを窓に向ける** — `ram_address_select_n` は
   既に `A0000-A7FFF` を SDRAM から外してあるので、
   `tvram_mem_select` と同じ形で読み出しの mux に足すだけ
3. **SDRAM の3本目のマスタ** — 現在 shim は port A(ゲスト)と
   port B(行バッファ)の2本。CG ウィンドウにも要るので、
   `sdram_mp` を PORTS=3 にして shim に port C を出すか、
   **port B を行バッファと調停して共有**する。
   後者のほうが安いが、行バッファは表示中ずっと使うので優先度の設計が要る

> 注意: 映像側のポートを増やすと SDRAM の消費が遅くなる。
> **`DROP` が 0 のままか必ず確認すること**(§1.0 の危険)。

### 1.2c ★ ファームウェアをスロット化した(2026-09-09)

**計測器の変更が15〜20分のビルドからファイルコピーになった。**

実測: Quartus ジョブは 898〜1189秒、キューは3秒。
今夜のビルドの何回かは `postmon.c` しか変えていないのにこれを全部払っていた。
`firmware.vh` が `$readmemh` でビットストリームに焼かれているため。

- ソフトコアの ROM に書き込みポートを足し、データスロットが起動時に上書きする
- **焼き込み版は ROM の初期値として残る** → ファイルが無いカードは今までどおり動く。
  ファイルを消せばコミット済みの版に戻る
- ソフトコアは起動時ダウンロードが終わるまでリセット保持
  (`reset_soft` は 65535 サイクル ≒ 1.5ms の固定待ちで、スロットのロードより
  ずっと短かった)。**ワンショット**なので、後からフロッピーをマウントしても
  OSD が消えない

**使い方**: `scripts/push_firmware.sh` — ビルド → レイアウト検査 → コピー → イジェクト。

### 1.2d 計測器のレイアウトを自動検査するようにした

PC-98 の表示が読めない事故が3回起きたので、目視をやめた。
`scripts/check_osd_layout.py` が `config.tcl` のマクロで
**実際にビルドされる構成**をプリプロセスし、8pxフォントから各フィールドの
占有幅を計算して重なり・画面外を検出する。CI 実行。

見つかった本物の問題: `SEQ` と `IO` の衝突、`LDN` と `MAX` の衝突、
**`DROP`/`HW` がパネル右端をはみ出していた**(`BANK` も巻き添え)。
ポート0x80系は PC/AT 専用、`IO`/`LDN` は PC-98 専用に分離した。
