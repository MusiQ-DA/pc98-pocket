...
| **rowbuf をレンダラに接続** | ⬜ **次。CDC が要る(§1.2b)** |
| CG ウィンドウ `A4000-A4FFF` | ⬜ |

#### 1.2b 残り作業と、なぜ止めたか

`pc98_glyph_rowbuf` を `PERIPHERALS` に置いてレンダラに繋ぐのが最後の一歩だが、
**クロックドメインをまたぐ**:

| 側 | クロック |
|---|---|
| TVRAM の読み出し / SDRAM の port B / rowbuf の充填 | `clock`(chipset、42.95MHz) |
| ラスタ / レンダラ / rowbuf の読み出し | `clk_vga_cga`(ドット、21.05MHz) |

必要なもの:

1. **`fill_start` の CDC** — テキスト行の先頭(ドットドメイン)で発生し、
   充填FSM(チップセットドメイン)を起動する。**パルスの受け渡しなので
   トグル同期が要る**。単純な `synch_3` では取りこぼす
2. `row_base` はパルスと一緒に渡す(充填中は変わらないので同期不要)
3. `bank` の切り替わりが読み出し側に見えるタイミング — 行境界でしか変わらないので
   `synch_3` で足りるが、**充填完了と行の切り替わりの順序**を確認すること
4. レンダラの `font_row` を ANK BRAM から rowbuf に切り替える
   (ANK BRAM は CG ウィンドウ用に残すか、削るか判断)

**CDC を急いで書かない判断で止めた。** 間欠故障は今週いちばん高くついた失敗の型で、
CDC のバグはまさにそれを生む。部品側は全部テスト済みなので、
残りは接続とCDCだけ。
