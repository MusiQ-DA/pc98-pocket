...
| `pc98_cgwindow`(`A4000-A4FFF`) | ✅ **接続済み**(SDRAM port C) |
...
| ポート | 用途 | 稼働 |
|---|---|---|
| A | ゲスト(`RAM.sv` 経由、KFSDRAM のプロトコル) | 常時 |
| B | 字形の行バッファ | 表示中ずっと |
| C | CG ウィンドウ(ゲストが字形を読む窓) | 문字を要求されたときだけ |

C を B と共有せず独立させたのは、**B が表示中ずっと動いている**ため。
共有すると「めったに要求しない側から表示を守る」優先度設計が要る。
3ポートのラウンドロビンのほうが単純で行儀もよい。

**コストは測った**(仮定していない): ローダは **7.25 クロック/バイト のまま**、
予算 10.9 に対して変化なし。他のマスタがアイドルなら調停はタダなので。
SDRAM 系ベンチは `tb_cpu_timing` の3種の far end を含め全部 PASS。

> それでも実機では **`DROP` が 0 のままか確認すること**。
> シミュレーションのアクセスパターンは実機の全部ではない。
