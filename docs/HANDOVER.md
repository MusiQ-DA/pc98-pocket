...
2. ~~**P0 手順5: 実機統合の下準備**~~ ✅ **完了**
   ...
   - シムは 42.95MHz 据え置き。85.9MHz 化はクロック配線が CHIPSET〜core_top まで
     ...
     **testB3 実機テスト(SD投入済み・実施待ち)— 分岐の実験**
     - run#49 = **現行ツリー + 純KFSDRAM**(SDRAM_USE_MP コメントアウト)。
       12,049 ALM = 動作実績のB1と全く同一。階層に shim/mp なし
     - **判定**:
       - ✅ BIOS が出る → 失敗は mp/shim 固有(全シミュレーションの外側)。
         次はハードウェア・バイセクト: KFSDRAM の FSM を shim 配下にそのまま
         移植した「同一クロック同一構成」から始め、実機で1ステップずつ mp へ変形
       - ❌ BIOS が出ない → run#39 以降のツリー変更(RAM.sv の ifdef 編集や
         ビルド環境)が原因。コミットバイセクトで特定
