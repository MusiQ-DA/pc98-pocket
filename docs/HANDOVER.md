...
### 1.0 ⚠️ まず読むこと: 計測器が実験を汚染していた

`postmon.c` の `guest_peek()` は **`ext_access_request` でバスを奪う**。
testB21 でこれを**毎ループ実行**するようにしたため、
**BIOS がメモリテストを走らせている最中に横から SDRAM を叩いていた**。
...
**→ §1.1 の「間欠だから物理」という結論は、比較対象が違う(ビルドが違う)ので無効。**
間欠だったのではなく、**計測器を足した時点で壊れた**可能性が高い。

対策: ベクタ読み出しを「**POST の進行が止まってから1回だけ**」に限定した(testB24)。
`VEC -- RUNNING` と表示されている間は読んでいない。

**教訓: バスを奪う計測器は、測っている対象を壊しうる。**
観測専用(`post_monitor.sv`)とバスを奪う観測(`guest_peek`)は別物として扱うこと。

### 1.1 (無効の疑い)実機データ(POSTモニタ、`post_monitor.sv`)
