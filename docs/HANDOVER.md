...
1. **タイマはまだ一度もアームされていない**(`PIT WR ctrl <= 36` が0件)。
   `F44CB`/`F45CB` の INT 1Ch AH=02 は**どちらも IRET で終わるハンドラの中**
   で、BASIC が `CS:12B0` に設置したタイマハンドラの**自己再アーム**。
   最初にアームする場所を特定すること。§3.2 の3修正(8259エッジ検出・
   2.4576MHz・IRR0クリア)の検証点はそこ。
2. 対話性の確認: `+keys=3\r\w\w\wPRINT_2\r` で `Ok` にコマンドを打ち、
   画面に `2` が出るか。キーボード→BIOS→BASIC→画面が閉じる。
3. `+golden` と PIT シードのフックは FE1FD で死んでいる(§3.5) — 撤去してよい。
4. 実機デプロイ: `bash scripts/deploy_pc98.sh`。**`memsw[3]` の修正が入って
   いること**を確認する(§1.2b) — 入っていないと実機でもバナー直後に落ちる。
