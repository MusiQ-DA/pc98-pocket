...
### 1.1 ★ 現在の作業: PC-98(P1)。ITF が動いていない

`MACHINE_PC98` ビルドの実機結果(run#114):

```
LIVE 07E83（停止）   HIGH FFFFF
```

- `HIGH FFFFF` は**リセットベクタが読まれた証拠にしかならない**
  (CPU は起動時に必ずそこを読む)
- `LIVE` が低位メモリで停止 = **未初期化 RAM を実行している = 暴走**

**最有力: ITF がシャドウバンクに書かれておらず、`FFFF0` でゴミを読んで飛んだ。**

#### 次のビルドで見る値

ROM スヌープを `FFFF0-FFFFF` に向けた(`MACHINE_PC98` 時)。
`LD` はゲストアドレスを見るので、**BIOS(通常バンク)と ITF(シャドウ)の両方**が
この窓に書き込みに来る:

| `LDN` | 意味 |
|---|---|
| **16** | **BIOS しか書いていない = ITF スロットが一度もロードされていない** |
| **32** | 両方書いた。ITF はロード済みで、原因は別 |

| `LD` / `RD` | 意味 |
|---|---|
| 両方 `EA 00 00 00 F8` | ITF は正しく入り、正しく読めている |
| `LD` 正・`RD` 誤 | メモリが保持していない |
| `LD` がゴミ | ローダが書いていない |

#### 既知の小バグ(未修正、原因ではないはず)

`core_top` の `bios_load_state` 02 で `tandy_bios_write <= select_shadow` は
**live の `ioctl_addr`** を読む。コピアはその頃 FIFO を pop 済みなので、
**次のエントリのアドレス**を見ている。連続領域では問題ないが、
**スロットの最後のワードだけ次スロットの判定になる** = ITF 末尾2バイトが
シャドウでなく通常バンクに行く。`FFFF0` は末尾なので**これが原因の可能性はある**。

### 1.2 PC-98 の実装状況

| 部品 | 状態 |
|---|---|
| メモリマップ / ROM 配置(BIOS 96KB → `E8000`) | ✅ |
| ITF バンク(ポート `0x043D`、`F8000-FFFFF`) | ✅ RTL・TB |
| `pc98_tvram`(`A0000` 文字 / `A2000` 属性) | ✅ ゲスト空間に接続済み |
| `pc98_font_ank`(8×16 ANK、`FONT.ROM` `0x0800-0x17FF`) | ✅ |
| `pc98_text_render`(属性デコード、色は **GRB 順**) | ✅ |
| `pc98_video_timing`(848×440 / 640×400 / 24,826Hz / 56.42Hz) | ✅ |
| `pll_video_pc98`(21.0526 MHz) | ✅ |
| 映像経路への接続 | ✅ commit 済み(未検証) |
| **漢字**(SDRAM 常駐 + ラインバッファ) | ⬜ **これが無いと BIOS 画面は完成しない** |
| CG ウィンドウ `A4000-A4FFF` | ⬜ |

### 1.3 計測器についての教訓(この日だけで5回踏んだ)

1. **取り込みエッジ** — 読みデータはサイクル末尾で有効。先頭でラッチすると1つ前が入る
2. **AEN の修飾** — ext アクセスは DMA と同じく AEN 高でゲストバスを駆動する。
   窓は `~address_enable_n` で修飾すること
3. **バスを奪う計測は対象を壊す** — `guest_peek` は `ext_access_request` を上げる
4. **PC/AT 固有のゲートに PC-98 の表示を入れた** — `post_max >= 0x08` は
   ポート 0x80 の進捗コードで、PC-98 では永久に開かない。
   **受動レジスタの読み出しをゲートする理由は無い**
5. **表示が画面からはみ出る/重なる** — パネルは 320px = 40桁。
   計測器は「読めて初めて計測器」

### 1.4 ✗ 外れた対策(繰り返さないこと)

- ✗ **PLL 位相 11640 → 8730 ps**: setup が**両方悪化**(read -2.357 → -4.585)。戻した。
  位相は 1455 ps の整数倍のみ合法(VCO 687.2727 MHz)
- ✗ **CAS レイテンシ 3**: STA も実機も不変。この制約はピン間関係で CL に依存しない
- ✗ **`-max 3.5` への較正**: STA 悪化。5.9 に戻した

> 読み出しパスの STA `-2.357ns` は **KFSDRAM も同値でそちらは起動する**ので、
> 「動く/動かない」を分けていない。hold 側には 11〜16ns 余っている。
