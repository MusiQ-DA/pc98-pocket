...
**sdram_mp でも BIOS は動き出し、最初の起動音までは到達する。
しかしその後に続くはずの2つ目のジングルが鳴らず、画面も黒いまま止まる。
= 起動シーケンスの途中で死んでいる。メモリ系の疑いは晴れていない。**
...
**さらに決定的**: 動く KFSDRAM では上昇3音の**あとに「Intel のサウンドロゴ」風の
ジングルが続く**。sdram_mp では**3音までで、その先が鳴らない**。
→ **実行は2つの音の「間」で止まっている。** 起動しきってはいない。
...
1. **BIOS ROM を逆アセンブルして2つの音の間の処理を特定する。**
   ROM は `~/Documents/lodemnc/np2rom/` ではなく PCXT 用のもの。
   典型的にはこの区間がメモリテスト本体。
2. **セルフテストを「BIOSと同じ速度・同じパターン」に近づける。**
   現在のセルフテストは1アクセス約16µs と遅く、それでは PASS する(testB14)。
   実機で落ちるのは連続アクセス時の可能性が高い →
   ソフトコアから最速で叩くモード、read-after-write、行/バンクを跨ぐ連続アクセスを足す
3. `refresh_mode` 変更の A/B(§1.4)
4. `tb_cpu_timing` の較正(§1.2-2)
