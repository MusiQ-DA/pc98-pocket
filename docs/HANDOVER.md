...
**N88-BASIC は起動している。画面に起動プロンプトとファンクションキー行が出ている。**

テキストプレーン(A0000-A1FFF)への書き込みを再構成した画面:

```
 0 |How many files (0-15)?UUUUUU...                                               |
 ...
24 |    load" auto  go to  list   run       save" key    print  edit . cont       |
```

(`U` = 0x55 は ITF のメモリテストパターン。ベンチは VRAM をクリアしないので
背景として残っているだけ。row 24 は N88-BASIC の F1-F10 ラベル行そのまま。)
...
✅ BASIC内部初期化(F353C-F3792)
✅ SS=0060 DS=0060 ES=A000 SP=F7E0
✅ **ファンクションキー行を描画**
✅ **"How many files (0-15)?" を表示**
─────────── ここまで動く ───────────
⏸  キー入力待ちで停止(F3563/F356A/F3C4A のポーリングループ、9800万回)
```

**「タイマー割込み待ちでブロック」は誤診だった。** F3563 のループは
キーボード入力待ちで、タイマーとは無関係。計測の根拠は §3.7。

### §1.1 キー入力は通った(2026-09-13 計測)

`+keys=3\r` で guest は**4バイト全部読み取った**(make 03 / break 83 /
make 1C / break 9C)。IRQ1・INT 09 ハンドラ・ワークエリアバッファが全て
生きている。**マスタPICの IRQ1 は `1'b0` に結線されていた**ので、それまでは
8251 が何を返してもキーは届かなかった(commit caf056a で修正)。

打った `3` は **row 0 の col 22 にエコーされた** — `How many files (0-15)?`
は22文字なので桁も正しい。キーは BIOS → BASIC → 画面まで往復している。

### §1.2 残る症状: 文字が書かれた直後に同一セルが 0x20 で潰される

3回連続の画面ダンプ(6秒間)で安定した定常状態:

```
00 |                      3                       |   ← エコーした '3' だけ残る
03 |                e                             |   ← "Bytes free" の末尾 e だけ
24 |    load " auto   go to  list   run.      save " key    print  edit . cont.  |
```

row 0 の書き込みを時系列で並べると例外なく:

```
    +0.0us  cell 00 <= 48 'H'
   +67.6us  cell 00 <= 20 ' '     ← 同一セル
   +95.0us  cell 01 <= 6F 'o'
  +162.1us  cell 01 <= 20 ' '     ← 以下22セル全部
```

**一方 row 24 は同じ出力ルーチンで書かれ、22us/文字・後続書き込みなしで
生き残る。** 同じルーチン、異なる呼び出し経路。

| | 1文字あたり | 後続の 0x20 | 結果 |
|---|---|---|---|
| row 24(F-key行) | 22us | なし | 残る |
| row 0(プロンプト) | 95us | あり | 潰される |
...
1. **`keys6` のログで 0x20 を書いている命令を特定する**(§1.2/§1.3)。
   EU ライブ PC が入ったので、文字ストアの直後に同一セルを潰している
   命令が名指しされる。row 24 が生き残る理由との差分が答え。
2. 特定後、BASIC が INT 1Ch AH=02 で 100Hz タイマを起動する地点まで進む
   (§3.7)。**そこが §3.2 の3修正(エッジ検出・PITクロック・IRR0クリア)の
   本当の検証点。**
3. `+golden` と PIT シードのフックは FE1FD で死んでいる(§3.5) — 張り直すか
   撤去する。素のブートで BASIC が出たのでどちらも不要な可能性が高い。
4. 実機デプロイ: `bash scripts/deploy_pc98.sh`(ROM は md5 ピン留め済み)。
   ただし §1.2 が未解決のままだと**実機でも画面は空白になる**はず —
   先に sim で潰す方が安い。
