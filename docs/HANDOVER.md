...
**testB7b(#58)で実機の症状が変わった: スプラッシュ → 「ピロリ」と発音 → 停止。**
**testB6 までは完全に無音だったので、8088 が BIOS POST を実行して失敗を報告する
ところまで前進している**(= SDRAM は「全く動かない」から「データが壊れる」段階へ)。
...
| **testB8** | — | mp + 較正済み入力遅延(3.5ns) | 未着手 | §1.9 |
...
1. **書き込みデータのサンプル時刻**。RAM.sv は `latch_data <= internal_data_bus` を
   **毎サイクル無条件で**取り込む。KFSDRAM は write_request の**1サイクル後**に
   WRITE を出して `data_in` を掴むが、sdram_mp は
   req(R)→ACT(R+1)→S_ACT→S_RW→WRITE(**R+5**)で、**5サイクル遅れて掴む**。
   その間に `internal_data_bus` が変われば**書く値が壊れる**。§1.3 のレイテンシ差と同根。
   → 検証法: shim 側で req を受けた瞬間に `data_in` をラッチして `p_wdata` に渡す
   (1行で済む。KFSDRAM と同じ掴み時刻になる)
...
3. **アドレス写像の違い**(機能的には全単射だが、実機で確かめた事はない)。
   KFSDRAM `{bank[23:22], row[21:9], col[8:0]}`(実質バンク0のみ・8192行)
   vs mp `{row[23:11], bank[10:9], col[8:0]}`(4バンク・2048行)。
   → 検証法: mp の写像を KFSDRAM と同一にする(パラメータ化かビット並べ替え1箇所)
