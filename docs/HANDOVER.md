...
### 3.6c 答え — ITF と BIOS が対になっていない(2026-09-14)

`tb_pc98_boot` にバンク切替の瞬間のメモリダンプを入れて確定した。

**低位 RAM にスタブなど無い**(`008D0` は 32 バイトとも 00)。`OUT 043D,12` は
**ROM の F88DB でそのまま実行されている**:

```
F88D6  BA 3D 04   mov dx,043D
F88D9  B0 12      mov al,12
F88DB  EE         out dx,al     ← ここから先のフェッチは BIOS イメージから来る
F88DC  FC B8 00 F8 8E D8 ...    ← ITF はこれが続くつもりでいる(自分の checksum ルーチン)
```

ところが BIOS イメージの F88DC は `E6 6A`(`out 6A,al`)で、まったく別のコード。
**命令ストリームが CPU の足元で入れ替わる。** だから暴走してゼロを実行し、
やがて `OUT 043D,10` で ITF に戻って最初からやり直す。実機の
「MEMORY 000KB OK → 再起動」はこれ。

決定的な測定:

| | |
|---|---|
| ITF の切替直後の続き | `FC B8 00 F8 8E D8 33 D2 ...` |
| BIOS イメージ 96KB 全体に同じ列 | **無し**(先頭 8 バイトでも無し) |
| 切替列 `BA 3D 04 B0 12 EE` | ITF に 1 箇所、**BIOS に 0 箇所** |

実機の PC-98 では ITF と BIOS は対になっていて、この番地に同じ続きが載って
いるから切り替えが透過になる。このツリーの ROM は対になっていない。

**RTL のバグではない。** SDRAM も `v30_cpu_bridge` も実メモリ経路も無関係
(フラットメモリのベンチで再現する)。**直すには一台の実機から採った ITF +
BIOS の対が要る。**

**手元の 2 本の素性**(文字列から特定、2026-09-14):

| | 文字列 | 世代 |
|---|---|---|
| `itf.rom` | `COPYRIGHT (C) NEC CORPORATION 1982,1987` | **1987**(PC-9801UX 世代) |
| `bios.rom` | `NEC N-88 BASIC(86) version 2.0` / `Copyright (C) 1983` | **1983**(VM 以前) |

**4 年離れている。** 必要なのは**同じ PC-9801UX から採った 96KB の BIOS ROM**。
逆向き(この 1983 年の BIOS に合う ITF を探す)は現実的でない——その世代の機械は
この形の ITF を持たない。

`bios.rom` の構造自体は正しい(上位 32KB に `PARITY ERROR - BASE MEMORY` や
キーボード配列表があり、リセットベクタも `jmp FD80:0000`)。世代だけが合わない。

**これは 2 度目である。** `docs/FRANKEN_ROM_LESSON.md` が 1 度目
(混成イメージが CPU の幻のバグを作り、日単位を溶かした)。同文書が求めていた
「数分で検出する方法」を `scripts/check_rom_pair.py` として実装し、
`deploy_pc98.sh` の門に入れた。md5 の照合は「各ファイルが名乗りどおりか」は
言えても「同じ機械から採られたか」は言えない。
