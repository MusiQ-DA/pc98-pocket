...
2. ~~**P0 手順5: 実機統合の下準備**~~ ✅ **完了**
   ...
   - シムは 42.95MHz 据え置き。85.9MHz 化はクロック配線が CHIPSET〜core_top まで
     ...
     **testB2 実機テスト(SD投入済み・実施待ち)**
     ...
     - **判定**:
       - ✅ BIOS が testB1 と同様に出る → **sdram_mp が実機で動作確定。
         P0 完了 → 手順4の `gram_cache.sv` 実装(G-RAM SDRAM化の本体)へ**
       - ❌ BIOS が出ない → 2バグ修正でも動かない。次は
         (a) `config.tcl` の `SDRAM_USE_MP` 行をコメントアウトした KFSDRAM 版を
         ビルドして回帰確認(統合パスの問題切り分け)、
         (b) TB のバスモデルを実 CHIPSET 波形に近づける、の順
       - ❌ Load error → パッケージ/変換の問題に戻る(今回 bitrev 検証済みなので
         可能性は低い。その場合は JSON 差分を再確認)
