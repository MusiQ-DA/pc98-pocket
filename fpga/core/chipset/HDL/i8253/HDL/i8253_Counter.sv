...
module i8253_Counter #(
    ...
    always_ff @(posedge clock, posedge reset) begin
        if (reset)
            ...
            if (select_read_write != `RL_SELECT_LSB_MSB)
                ...
                casez (select_mode)
                    `i8253_CONTROL_MODE_0: begin
                        if (write_count_step == 1'b0)
                            start_counting <= 1'b1;
                        else
                            start_counting <= 1'b0;
                    end
                    `i8253_CONTROL_MODE_4: begin
                        if (write_count_step == 1'b0)
                            start_counting <= 1'b1;
                        else
                            start_counting <= 1'b0;
    ...
    always_comb begin
        ...
        casez (select_mode)
            ...
            `i8253_CONTROL_MODE_1: begin
                if (gate_edge)
                    count_next = count_preset_load;
            end

            `i8253_CONTROL_MODE_2: begin
                if (counter_gate == 1'b0)
                    count_next = count;
    ...
    always_ff @(posedge clock, posedge reset) begin
        if (reset)
            ...
            casez (select_mode)
                `i8253_CONTROL_MODE_0: counter_out <= 1'b0;
                `i8253_CONTROL_MODE_1: counter_out <= 1'b1;
                `i8253_CONTROL_MODE_2: counter_out <= 1'b1;
                `i8253_CONTROL_MODE_3: counter_out <= 1'b1;
                `i8253_CONTROL_MODE_4: counter_out <= 1'b1;
                `i8253_CONTROL_MODE_5: counter_out <= 1'b1;
